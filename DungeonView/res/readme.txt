...
•	In the terminal type: java –jar DungeonView.jar <row> <col> <type> <interconnectivity> <cave with treasures> <Monsters> e.g. java -jar DungeonView.jar 5 5 1 3 0.6 4     .Enter 1 for Wrapping
	and 2 for non-wrapping dungeon. If no command-line argument given then GUI based game is launched.
...
After the program starts running the user will be asked to enter an option to play the game which are as follows:
			...
			* The user can pick arrow by pressing "A";
			* The user can pick Treasure by pressing "T"
			* The user can shoot an Arrow by pressing "S" followed by and arrow key and giving the distance in the input dialog box. 	
			* The user can move to the neighbouring cell by click on them or by using the arrow keys. 	
Depending on the choice of the user the description and movement of the player in the maze is displayed on the console in the text-based version of the game. If the user does not select of the given option
...
•	The visited flag has been introduced to mark the visited cells.
•	The IDungeon Interface has been split into read-only IDungeonReadOnly and mutating IDungeon interfaces.
...
•	The Program is occassionaly slow to render.
•	The Program does not support maximization of the window
